...
class BrandServiceTest {
    ...
    void testFindByIdentifier_Success() {
        ...
        verify(modelMapper, times(1)).map(brand, BrandDto.class);
    }
    ...
    void testFindByIdentifier_NotFound() {
        ...
        verify(modelMapper, times(1)).map(null, BrandDto.class);
    }
    ...
    void testFindAll_WithData() {
        ...
        assertEquals(brandDto.getIdentifier(), result.getDtoList().get(0).getIdentifier());
    ...
    void testFindActiveBrands() {
        ...
        when(brandRepository.findByStatus(true)).thenReturn(List.of(brand));
        ...
        verify(brandRepository, times(1)).findByStatus(true);
    }

    @Test
    void testDelete_Success() {
        doNothing().when(brandRepository).deleteByIdentifier("Admin");

        assertDoesNotThrow(() -> brandService.delete("Admin"));

        verify(brandRepository, times(1)).deleteByIdentifier("Admin");
    }
    ...
    void testToggleStatus_BrandNotFound() {
        ...
        verify(brandRepository).findByIdentifier("Admin");

        verify(brandRepository, never()).save(any());

        verify(modelMapper, never()).map(any(), eq(BrandDto.class));
    }
    ...
    void testToggleStatus_FromFalseToTrue() {
        ...
        assertNotNull(result);
        assertTrue(brand.isStatus()); // flipped to true
