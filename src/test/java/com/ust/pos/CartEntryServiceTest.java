...
class CartEntryServiceTest {
    ...
    void setUp() {
        ...
        cartEntryDto.setIdentifier("PROD1_CART1");
    ...
    void testFindByIdentifier_Found() {
        ...
        CartEntryDto result = cartEntryService.findByIdentifier("PROD1_CART1");

        assertNotNull(result);
        verify(cartEntryRepository).findByIdentifier("PROD1_CART1");
    }

    @Test
    void testFindByIdentifier_NotFound() {
        when(cartEntryRepository.findByIdentifier("PROD1_CART1")).thenReturn(null);
        CartEntryDto result = cartEntryService.findByIdentifier("PROD1_CART1");
        assertNull(result);
    }
    ...
    void testGetSellingPrice() {
        when(priceRepository.findByProductAndPriceTypeAndDeletedFalse("PROD1", "Selling Price")).thenReturn(sellingPrice);
    ...
    void testGetDiscount() {
        when(priceRepository.findByProductAndPriceTypeAndDeletedFalse("PROD1", "Selling Price")).thenReturn(sellingPrice);
        when(priceRepository.findByProductAndPriceTypeAndDeletedFalse("PROD1", "MRP")).thenReturn(mrpPrice);

        BigDecimal result = cartEntryService.getDiscount(cartEntryDto);
        assertEquals(BigDecimal.valueOf(100), result);
    }
    ...
    void testSave_NewCartEntry() {
        ...
        when(cartEntryRepository.findByIdentifier("PROD1_CART1")).thenReturn(null);
        when(modelMapper.map(cartEntryDto, CartEntry.class)).thenReturn(cartEntry);

        when(cartEntryRepository.findAllByCartId("CART1")).thenReturn(Collections.emptyList());
    ...
    void testSave_ExistingCartEntry() {
        ...
        when(priceRepository.findByProductAndPriceTypeAndDeletedFalse("PROD1", "MRP"))
                .thenReturn(mrpPrice);

        when(priceRepository.findByProductAndPriceTypeAndDeletedFalse("PROD1", "Selling Price"))
        ...
        verify(modelMapper).map(result, existing);
    ...
    void testRecalculate_EmptyEntries() {
        ...
        assertEquals(0, cart.getDiscount().compareTo(BigDecimal.ZERO));
        verify(cartRepository).save(cart);
    }
    ...
    void testDelete() {
        ...
        verify(cartEntryRepository).deleteByIdentifier("PROD1_CART1");
        verify(cartRepository).save(cart);
    }
    ...
    void testFindAll() {
        ...
        WsDto<CartEntryDto> result = cartEntryService.findAll(pageable);
