...
class CartEntryServiceTest {
    ...
    // ✅ Validation: productId missing
    @Test
    void testSave_ProductIdMissing() {
    ...
    // ✅ Price missing
    @Test
    void testSave_PriceNotConfigured() {
    ...
    // ✅ Save NEW entry (selling only)
    @Test
    void testSave_NewEntry_SellingPriceOnly() {
    ...
    // ✅ Save NEW entry (MRP only)
    @Test
    void testSave_NewEntry_MrpOnly() {
    ...
    // ✅ Discount calculation (MRP - SELLING)
    @Test
    void testSave_DiscountCalculation() {
        ...
        verify(cartEntryRepository).save(argThat(entry ->
                entry.getDiscount().compareTo(BigDecimal.valueOf(20)) == 0
        ));
    }

    // ✅ Negative discount protection
    @Test
    void testSave_DiscountNeverNegative() {
        ...
        verify(cartEntryRepository).save(argThat(entry ->
                entry.getDiscount().compareTo(BigDecimal.ZERO) == 0
        ));
    }

    // ✅ Existing entry → quantity update
    @Test
    void testSave_UpdateExistingEntry() {
    ...
    // ✅ unitPrice null → default ZERO
    @Test
    void testSave_UnitPriceNullHandling() {
        ...
        verify(cartEntryRepository).save(argThat(entry ->
                entry.getUnitPrice().compareTo(BigDecimal.ZERO) == 0
        ));
    }

    // ✅ findByIdentifier - found
    @Test
