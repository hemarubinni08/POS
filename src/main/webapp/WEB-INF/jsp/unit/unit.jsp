...
<head>
    ...
    <title>Retail POS | Edit Unit of Measure</title>
...
<body>
    ...
    <header class="top-navbar">
        <a href="${pageContext.request.contextPath}/unit/list" class="btn-back">
    ...
    <main class="main-content">
        <div class="form-card">
            <div class="mb-4">
                <h2 style="font-weight: 800; color: var(--primary-navy); margin: 0;">Edit Unit</h2>
                <p style="color: var(--text-muted); font-size: 14px;">Updating record for: <strong class="text-primary">${unitDto.identifier}</strong></p>
            </div>

            <form:form id="editUnitForm" method="POST" action="update" modelAttribute="unitDto">

                <%-- Hidden ID for JPA update logic --%>
                <form:hidden path="id" />

                <div class="form-group">
                    <label>Unit Identifier (Fixed)</label>
                    <%-- Read-only to maintain consistency with Categories and Brands --%>
                    <form:input path="identifier" class="input-control input-readonly" readonly="true"/>
                </div>

                <div class="mt-4" style="font-size: 12px; color: var(--text-muted); background: #f8fafc; padding: 15px; border-radius: 10px; border: 1px dashed var(--border-color);">
