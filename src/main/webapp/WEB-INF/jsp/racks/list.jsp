...
<head>
    ...
    <title>Retail POS | Racks Management</title>
    ...
    <main class="main-content">
        <div class="data-card">
            <div class="card-header-custom">
                <div>
                    <h4 class="m-0" style="font-weight: 800; color: var(--primary-navy);">Racks Management</h4>
                    <p class="m-0 text-muted" style="font-size: 13px;">Overview of sub-storage locations and assignments.</p>
                </div>
                <a href="${pageContext.request.contextPath}/racks/add" class="btn btn-primary"
                ...
                <table class="table table-hover align-middle m-0">
                    <thead>
                        <tr>
                            ...
                            <th>Racks Identifier</th>
                            <th>Assigned Shelfs</th>
                            <th>Status (Toggle)</th>
                        ...
                        <c:choose>
                            <c:when test="${fn:length(racks) > 0}">
                                <c:forEach var="r" items="${racks}">
                                    <tr>
                                        <td class="ps-4">
                                            <div style="font-weight: 600; color: var(--text-muted);"># ${r.id}</div>
                                        </td>
                                        ...
                                        <td>
                                            <span class="shelfs-badge">
                                                <c:out value="${r.shelves}" />
                                            </span>
                                        </td>
                                        ...
                                        <td>
                                            <div class="form-check form-switch d-flex align-items-center">
                                                ...
                                                <label class="ms-2 status-label ${r.status ? 'text-active' : 'text-inactive'}">
                                        ...
                                        <td class="text-end pe-4">
                                            <div class="d-flex justify-content-end gap-2">
                                                ...
                                                <a class="btn btn-outline-danger btn-sm px-3 btn-op"
                                                   href="${pageContext.request.contextPath}/racks/delete?identifier=${r.identifier}"
                                                   onclick="return confirm('Delete this rack record?');">Delete</a>
                                            </div>
                                        </td>
                                    </tr>
                                </c:forEach>
                            ...
                            <c:otherwise>
                                <tr><td colspan="5" class="text-center py-5 text-muted">No racks defined yet.</td></tr>
                            </c:otherwise>
                        </c:choose>
                    </tbody>
                </table>
    ...
    <script>
        ...
        document.querySelectorAll('.status-toggle').forEach(toggle => {
            toggle.addEventListener('change', function() {
                ...
                .then(response => {
                    if (response.ok) {
                        toast.innerText = "Rack Status Updated Successfully";
                        ...
                        label.innerText = !isChecked ? 'ACTIVE' : 'INACTIVE';
                        label.className = `ms-2 status-label ${!isChecked ? 'text-active' : 'text-inactive'}`;
                        alert("Error: Database update failed for rack status.");
                    }
                })
                .catch(err => {
