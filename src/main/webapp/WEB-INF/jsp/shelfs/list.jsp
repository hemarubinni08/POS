...
<head>
    ...
    <title>Retail POS | Shelfs Management</title>
    ...
    <main class="main-content">
        <div class="data-card">
            <div class="card-header-custom">
                <div>
                    <h4 class="m-0" style="font-weight: 800; color: var(--primary-navy);">Shelfs Management</h4>
                    <p class="m-0 text-muted" style="font-size: 13px;">Manage storage locations and organizational units.</p>
                </div>
                <a href="${pageContext.request.contextPath}/shelfs/add" class="btn btn-primary"
            ...
            <div class="table-responsive">
                <table class="table table-hover align-middle m-0">
                    ...
                    <tbody>
                        <c:choose>
                            <c:when test="${fn:length(shelfs) > 0}">
                                <c:forEach var="s" items="${shelfs}">
                                    <tr>
                                        <td class="ps-4">
                                            <div style="font-weight: 600; color: var(--text-muted);"># ${s.id}</div>
                                        </td>
                                        ...
                                        <td>
                                            <div class="form-check form-switch d-flex align-items-center">
                                                <input class="form-check-input status-toggle" type="checkbox" role="switch"
                                                       data-id="${s.identifier}"
                                                       ${s.status ? 'checked' : ''}
                                                       style="width: 34px; height: 17px;">
                                                <label class="ms-2 status-label ${s.status ? 'text-active' : 'text-inactive'}">
                                        ...
                                        <td class="text-end pe-4">
                                            <div class="d-flex justify-content-end gap-2">
                                                <a class="btn btn-outline-primary btn-sm px-3 btn-op"
                                                   href="${pageContext.request.contextPath}/shelfs/get?identifier=${s.identifier}">Edit</a>
                                                <a class="btn btn-outline-danger btn-sm px-3 btn-op"
                                                   href="${pageContext.request.contextPath}/shelfs/delete?identifier=${s.identifier}"
                                                   onclick="return confirm('Remove this storage unit?');">Delete</a>
                                            </div>
                                        </td>
                                    </tr>
                                </c:forEach>
    ...
    <script>
        ...
        document.querySelectorAll('.status-toggle').forEach(toggle => {
            toggle.addEventListener('change', function() {
                ...
                // Async request to update DB without redirecting
                fetch(`${pageContext.request.contextPath}/shelfs/toggle?identifier=` + identifier + `&status=` + isChecked, {
                    method: 'POST'
                })
                .then(response => {
                    if (response.ok) {
                        toast.innerText = "Shelf Status Updated Successfully";
