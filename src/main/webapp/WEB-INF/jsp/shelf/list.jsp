...
function toggle(identifier, checkbox) {
    fetch('<%= request.getContextPath() %>/shelf/toggle?identifier=' + identifier)
        .then(() => {
            showFireworks(checkbox);
        })
        .catch(err => {
            console.error(err);
...
function showFireworks(el) {
    ...
    const colors = ['#ff004f', '#00eaff', '#ffe600', '#8cff00', '#ff9100'];
